...
namespace System.Gpio.Devices.Utils
{
    /// <summary>
    ...
    public static class WeatherHelper
    {
        ...
        {
            var meters = (Math.Pow(seaLevelPressure.Pascals / pressure.Pascals, 1 / 5.255) - 1) * airTemperature.Kelvins / DefaultTemperatureGradient;
        ...
        /// to be used above ~750m). Do not use the height obtained by calling <see cref="CalculateAltitude(Pressure)"/>
        ...
        {
            var rs = SpecificGasConstantOfAir;
            var rd = SpecificGasConstantOfVapor;
            var pd = CalculateSaturatedVaporPressureOverWater(temperature);
            // It's still called "constant" even though it's not constant
            var gasConstant = rs / (1 - humidity.Percent / 100 * (pd.Pascals / airPressure.Pascals) * (1 - rs / rd));
        ...
        public static Temperature CalculateWindchill(Temperature temperature, Speed windSpeed)
        {
            ...
            var va = temperature.DegreesCelsius;
            ...
            var wct = 13.12 + 0.6215 * va + (0.3965 * va - 11.37) * Math.Pow(windSpeed.KilometersPerHour, 0.16);
        ...
        /// </summary>
        /// <param name="densityOfAir">The denisty of the air, calculated using one of the overloads of <see cref="CalculateAirDensity(Pressure,Temperature)"/></param>
        ...
        {
            var v = windSpeed.MetersPerSecond;
            var rho = densityOfAir.KilogramsPerCubicMeter;

            var wd = pressureCoefficient * rho / 2 * (v * v);
